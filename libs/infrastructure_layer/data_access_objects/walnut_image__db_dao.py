...
class WalnutImageDBDAO(Base):
    ...
    # Unique constraint
    __table_args__ = (UniqueConstraint("walnut_id", "side", name=CONSTRAINT_UQ_WALNUT_SIDE),)

    # Relationships
    # Using string literals - SQLAlchemy resolves them by class name at runtime
    walnut: Mapped["WalnutDBDAO"] = relationship("WalnutDBDAO", back_populates="images")
    embedding: Mapped[Optional["WalnutImageEmbeddingDBDAO"]] = relationship(
        "WalnutImageEmbeddingDBDAO", back_populates="image", uselist=False, cascade="all, delete-orphan", lazy="select"
    )
